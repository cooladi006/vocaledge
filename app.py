# -*- coding: utf-8 -*-
"""
VocalEdge Clarity Funnel — Automated Backend
Direct-upload scoring endpoint for the webpage-based funnel (the ManyChat DM
now just sends a link to the page; the page itself records audio, submits
here via /api/score, and renders the live result). The original
/webhook/voice-note endpoint is kept for reference but is no longer the
primary path.
"""
...
# ============ PILLAR -> VIDEO LOOKUP ============
# Maps a detected weak-sound category to the best-matching already-published lesson.
# Fill in the real YouTube URL once each video is confirmed live - title match is enough to ship with.
VIDEO_LOOKUP = {
...
# Phoneme -> pillar mapping (heuristic starting point - calibrate against real results after first ~20 users)
PHONEME_PILLAR_MAP = {
    "v": "v_w", "w": "v_w",
    "z": "z_s", "s": "z_s",
    "th": "th", "dh": "th",
...
# ============ CORE PIPELINE (legacy, used by the old ManyChat webhook path) ============

def score_with_azure(audio_url):
    """Download the user's voice note (by URL) and score it against the baseline paragraph via Azure Pronunciation Assessment."""
...
def find_weakest_pillar(azure_result):
    ...
    map it to one of our 5 pillars. Falls back to word_stress if no clear phoneme signal."""
    try:
        ...
        if phoneme_scores:
            ...
            overall = nbest.get("PronunciationAssessment", {}).get("AccuracyScore", 75)
            return weakest, round(overall)
        overall = nbest.get("PronunciationAssessment", {}).get("AccuracyScore", 75)
        return "word_stress", round(overall)
    except Exception as e:
        print(f"Scoring parse error, using safe fallback: {e}")
        return "word_stress", 75

# ============ CARD GENERATOR (kept for reference / potential future re-use, not on the primary path) ============
...
def handle_voice_note():
    """LEGACY - original ManyChat-relay path. Not used by the current webpage-based funnel,
    kept only in case DM-relay is ever revisited."""
    data = request.json
...
def api_score():
    """PRIMARY PATH. Direct browser upload endpoint for the scoring webpage
    (clarity_score_page.html). Accepts multipart/form-data with an 'audio'
    file and a 'name' field, returns JSON so the page can render its own
    live, animated score reveal."""
    ...
    try:
        ...
        print(f"api_score error: {e}")
        return _cors(jsonify({"error": "Could not score that recording. Please try again."})), 500
...
def capture_email():
    """Called by the webpage when a user drops their email after seeing their score -
    pushes into the same Formspree endpoint the website already uses, so DM/page leads
    and website leads land in one unified waitlist."""
    data = request.json
